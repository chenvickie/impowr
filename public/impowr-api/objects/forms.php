...
class FormControls
{
    ...
    public function read ($key = '', $value = '', $partial = 'No', $sort = 'id', $dir = 'asc', $offset = 0, $limit = 10, $jobId = 'all')
    {
        ...
        $noQuote = [ "f.job_id", "f.need_import" ];
        ...
        try {
            $formsQuery = "SELECT f.*, i.project_name, i.project_id, i.source_institution, i.source_project_name ";
            $totalQuery = "SELECT COUNT(*) as count ";

            $query = "FROM impowr_form_controls AS f
            ...
            $totalQuery .= $query;
            $formsQuery .= $query . " ORDER BY " . $sort . " " . $dir .
                " OFFSET " . $offset . " ROWS
                                    FETCH FIRST " . $limit . " ROWS ONLY";

            $dbh = $this->conn;
            $sth = $dbh->prepare ($formsQuery);
            ...
            while ( $row = $sth->fetch (PDO::FETCH_ASSOC) ) {
                ...
                $row['import_need'] = $row['import_need'] == 1;
            ...
            logs ("[ERROR] Error in read Forms: " . json_encode ($e->getMessage ()) . "<br/>", true, true);
    ...
    public function updateFormControl ($params)
    {
        try {
            $query = "UPDATE [dbo].[impowr_form_controls] SET  
                    import_need = ?,
                    date_activated = ? 
                    WHERE id = ?";
            ...
            logs ("[ERROR] Error in updateFormControl: " . json_encode ($e->getMessage ()) . "<br/>", true, true);
            return false;
        }
    }
}